...
namespace M3.Creating_types
{
    public class Methods
    {
        public static double FindNthRoot(double number, double degree, double accuracy)//можно ли округлять результат?
        {
            if (accuracy < 0 || degree < 0 || degree % 2 == 0 && number < 0)
                throw new ArgumentOutOfRangeException("It is impossible to extract an even degree approx from a negative number");

            var x0 = number / degree;
            var x1 = (1 / degree) * ((degree - 1) * x0 + number / Math.Pow(x0, degree - 1));
